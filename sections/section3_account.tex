...
続いてこのボタンを選択すると退会画面が表示され，一般会員は退会理由（任意）を入力し，削除確認項目にすべてチェックを入れる．
...
ここでキャンセルを選択した場合は退会画面に遷移し，「OK」を選択した場合はデータベースにアクセスして会員情報を削除し，ログイン画面へ遷移する．
...
質問や要望はメール形式で送信され，運営側からの回答は，一般会員が登録している Google アカウントのメールアドレスへ送信される．
...
ここで「申請」を選択せずにキャンセルを選択した場合は，マイページ画面へ遷移する．
