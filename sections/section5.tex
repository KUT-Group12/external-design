...
\subsection{ログアウト・ログイン機能}
\begin{figure}[H]
    \centering
    \includesvg[keepaspectratio, width=0.8\linewidth]{figures/admin_login.svg}
    \caption{ログイン機能}
    \label{fig:admin_login}
\end{figure}

ログイン機能では，管理者がGoogleアカウントを用いた認証によってシステムにログインする．ログインの処理は一般会員・事業者会員と同様でログイン認証画面で取得したアカウント情報がデータベースに登録済みである場合にログインを許可する．

\begin{figure}[H]
    \centering
    \includesvg[keepaspectratio, width=\linewidth]{figures/admin_logout.svg}
    \caption{ログアウト機能}
    \label{fig:admin_logout}
\end{figure}
ログアウト機能では，すでにログインしている会員が任意のタイミングでログアウト操作を行うことができる．ログアウト操作には，まずログアウト画面を表示し，ユーザーがキャンセルを選択した場合はホーム画面に遷移する．ログアウトを確定した場合はセッション情報を無効化し，ログイン画面へ遷移する．
